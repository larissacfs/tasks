...
export default class ShopCarsPage {
  ...
  constructor(page: Page) {
    ...
    this.colourMenuOption = page.locator(".category-filter-row p:text-is('Colour')")
    this.colourBtn = page.locator("a span:text-is(' Colour ')")
    this.sortingSelector = page.locator(".dcp-cars-srp__sorting-dropdown select")
    this.exploreAnByAnchorLink = page.locator("a span:text-is('Explore & Buy')")
    this.carYear = page.locator("span[data-test-id='dcp-cars-buy-box-vehicle-characteristics-model-year']")
    this.carVIN = page.locator("span:has-text('VIN') + span")
    this.speakToAnExpertBtn = page.locator("button[data-test-id='dcp-buy-box__contact-seller']")
    ...
    this.postalCodeInput = page.locator("label:has-text('Postal Code') + input")
    this.privacyCheckbox = "div[data-test-id='rfq-contact__consent-privacy'] input"
    this.smsDirectMarketingCheckbox = ".dcp-multi-checkbox__options input:nth-of-type(1)"
    this.proceedBtn = page.getByRole("button", { name: "Proceed"})
    this.emailErrorMsg = page.getByText("Please enter a valid email address using a minimum of six characters.")
  }
   ...
   * @param isBusiness 
   */
  async fillFormOut(firstName: string, lastName: string, email: string, phone: string, postalCode: string){
